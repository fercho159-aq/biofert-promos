const WHATSAPP_URL =
  "https://wa.me/5215529887336?text=Hola%2C%20me%20gustar%C3%ADa%20agendar%20una%20consulta.";

export default function Footer() {
  return (
    <footer id="contacto" className="bg-brand-800 text-white py-14" role="contentinfo">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          {/* Brand */}
          <div>
            <div className="mb-4">
              <img
                src="/biofert.svg"
                alt="Biofert - Clínica de Fertilidad"
                width={120}
                height={40}
                className="h-10 w-auto brightness-0 invert"
              />
            </div>
            <p className="text-gray-300 text-sm leading-relaxed">
              Clínica de fertilidad con más de 7,000 familias formadas. Te
              acompañamos en cada paso de tu camino.
            </p>
          </div>

          {/* Contact */}
          <div>
            <h2 className="font-semibold text-lg mb-4">Contacto</h2>
            <ul className="space-y-3 text-gray-300 text-sm">
              <li>
                <address className="not-italic">
                  Tlacotalpan 36, Roma Sur, Cuauhtémoc, CDMX. C.P. 06760
                </address>
              </li>
              <li>
                <a
                  href="tel:+5215529887336"
                  className="hover:text-white transition-colors inline-flex items-center gap-2 py-1"
                >
                  <span>Tel: 55 2988 7336</span>
                </a>
              </li>
              <li>
                <a
                  href={WHATSAPP_URL}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="hover:text-white transition-colors inline-flex items-center gap-2 py-1"
                >
                  Agendar mi consulta por WhatsApp
                </a>
              </li>
            </ul>
          </div>

          {/* Social / Links */}
          <div>
            <h2 className="font-semibold text-lg mb-4">Síguenos</h2>
            <div className="flex gap-2 flex-wrap">
              <a
                href="https://www.facebook.com/biofert.com.mx"
                target="_blank"
                rel="noopener noreferrer"
                aria-label="Visitar Facebook de Biofert"
                className="w-11 h-11 flex items-center justify-center rounded-full bg-brand-700 hover:bg-brand-600 transition-colors"
              >
                <svg
                  className="w-5 h-5"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                  aria-hidden="true"
                >
                  <path d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z" />
                </svg>
              </a>
              <a
                href="https://www.instagram.com/biofert_/"
                target="_blank"
                rel="noopener noreferrer"
                aria-label="Visitar Instagram de Biofert"
                className="w-11 h-11 flex items-center justify-center rounded-full bg-brand-700 hover:bg-brand-600 transition-colors"
              >
                <svg
                  className="w-5 h-5"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                  aria-hidden="true"
                >
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
                </svg>
              </a>
              <a
                href="https://x.com/Biofert_"
                target="_blank"
                rel="noopener noreferrer"
                aria-label="Visitar X de Biofert"
                className="w-11 h-11 flex items-center justify-center rounded-full bg-brand-700 hover:bg-brand-600 transition-colors"
              >
                <svg
                  className="w-5 h-5"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                  aria-hidden="true"
                >
                  <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                </svg>
              </a>
              <a
                href="https://www.tiktok.com/@biofert.reproduccion"
                target="_blank"
                rel="noopener noreferrer"
                aria-label="Visitar TikTok de Biofert"
                className="w-11 h-11 flex items-center justify-center rounded-full bg-brand-700 hover:bg-brand-600 transition-colors"
              >
                <svg
                  className="w-5 h-5"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                  aria-hidden="true"
                >
                  <path d="M12.53.02C13.84 0 15.14.01 16.44.02c.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.96-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.15 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07z" />
                </svg>
              </a>
              <a
                href="https://www.youtube.com/@biofert"
                target="_blank"
                rel="noopener noreferrer"
                aria-label="Visitar YouTube de Biofert"
                className="w-11 h-11 flex items-center justify-center rounded-full bg-brand-700 hover:bg-brand-600 transition-colors"
              >
                <svg
                  className="w-5 h-5"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                  aria-hidden="true"
                >
                  <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
                </svg>
              </a>
            </div>
          </div>
        </div>

        {/* Disclaimer */}
        <div className="mt-10 pt-8 border-t border-brand-700 text-center text-gray-400 text-xs leading-relaxed">
          <p>
            La información proporcionada en este sitio web es con fines
            informativos únicamente y no constituye consejo médico. Consulta
            siempre con un profesional de salud calificado. Los resultados
            pueden variar según cada caso individual.
          </p>
          <p className="mt-3">
            &copy; {new Date().getFullYear()} Biofert. Todos los derechos
            reservados.
          </p>
        </div>
      </div>
    </footer>
  );
}
